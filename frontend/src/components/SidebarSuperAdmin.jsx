...
import {
  LayoutDashboard, BookOpen, Users, GraduationCap, FolderArchive, UserCircle,
  Calendar, ClipboardList, Settings, BarChart3, CreditCard, Wallet, Receipt,
  ChevronLeft, ChevronRight, ChevronDown, LogOut, X,
} from 'lucide-react';
...
const NavItem = ({ label, Icon, isActive, collapsed, onClick, children, tooltip = true }) => {
  ...
  return (
    <div className="relative" onMouseEnter={(e) => collapsed && setRect(e.currentTarget.getBoundingClientRect())} onMouseLeave={() => setRect(null)}>
      <button
        onClick={onClick}
...
const Dropdown = ({ item, open, collapsed, pathname, onToggle, goTo }) => {
  ...
  return (
    <div onMouseEnter={(e) => collapsed && setRect(e.currentTarget.getBoundingClientRect())} onMouseLeave={() => setRect(null)}>
      <NavItem label={label} Icon={Icon} collapsed={collapsed} isActive={collapsed && isActive} onClick={onToggle} tooltip={false}>
        <ChevronDown size={14} className={`shrink-0 transition-transform duration-200 ${collapsed ? 'lg:hidden' : ''} ${open ? 'rotate-180' : ''}`} />
      </NavItem>
      ...
      {collapsed && rect && <Flyout item={item} anchorRect={rect} pathname={pathname} goTo={(p) => { setRect(null); goTo(p); }} />}
    </div>
  );
};

const SidebarSuperAdmin = ({ collapsed, setCollapsed, mobileOpen, onMobileClose }) => {
  ...
  const [open, setOpen] = useState(() => {
    let saved = {};
    try { saved = JSON.parse(localStorage.getItem('superadmin_sidebar_open')) || {}; } catch {}
    const a = findActive(pathname);
    return a ? { ...saved, [a.label]: true } : saved;
  });

  useEffect(() => { localStorage.setItem('superadmin_sidebar_open', JSON.stringify(open)); }, [open]);
  useEffect(() => { const a = findActive(pathname); if (a) setOpen((o) => ({ ...o, [a.label]: true })); }, [pathname]);
  ...
  const toggle = (item) =>
    collapsed && isDesktop() ? goTo(item.children[0].path) : setOpen((o) => ({ ...o, [item.label]: !o[item.label] }));
          ...
          {navGroups.map((group) => (
            <div key={group.label}>
              <div className={`overflow-hidden transition-all duration-300 ease-in-out ${collapsed ? 'lg:max-h-0 lg:opacity-0' : 'max-h-8 opacity-100 lg:delay-100'}`}>
                <p className="px-3 pt-3 pb-1 text-[10px] font-medium text-slate-400 uppercase tracking-wide whitespace-nowrap">{group.label}</p>
              </div>

              {group.items.map((item) =>
                item.children ? (
                  <Dropdown key={item.label} item={item} open={!!open[item.label]} collapsed={collapsed} pathname={pathname} onToggle={() => toggle(item)} goTo={goTo} />
                ) : (
                  <NavItem key={item.path} label={item.label} Icon={item.icon} collapsed={collapsed} isActive={item.path === '/admin' ? pathname === '/admin' : pathname.startsWith(item.path)} onClick={() => goTo(item.path)} />
                )
              )}
            </div>
          ))}
