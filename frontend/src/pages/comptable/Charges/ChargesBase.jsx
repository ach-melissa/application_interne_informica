...
const ChargesBase = ({ type }) => {
  ...
  const openEdit = (c) => { setEditingId(c.id); setForm({ categorie: c.categorie, montant: c.montant, date: c.date, description: c.description }); setFormError(null); setConfirm(null); setShowForm(true); };
  ...
  const doDelete = async () => {
    try {
      ...
      closeForm();
    } catch { setFormError('Échec de la suppression.'); }
  };
  ...
  const chargesDuMois = useMemo(() => charges
    ...
    .reduce((s, c) => s + Number(c.montant || 0), 0), [charges, categoryFilter, search, currentMonthKey]);

  const activeFilterCount = (search ? 1 : 0) + (categoryFilter ? 1 : 0) + (dateFrom || dateTo ? 1 : 0);
  const clearFilters = () => { setSearch(''); setCategoryFilter(''); setDateFrom(''); setDateTo(''); };
...
<div className="flex items-center gap-1.5 bg-white border border-[#E2E8F0] rounded-full pl-3 pr-1.5 py-1.5">
  ...
  <X size={11} /> Tout effacer
      ...
      <div className="flex flex-wrap gap-2 justify-end mb-5">
        <button onClick={() => setShowCategoryManager(true)} className="flex items-center gap-1.5 text-xs font-medium text-[#0369A1] bg-[#DCEBFA] hover:bg-[#c9e2f7] px-3.5 py-2 rounded-md transition">
      ...
      <ChargeFormModal
        show={showForm} onClose={closeForm} activeCategories={activeCategories}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs table-fixed">
            ...
            <thead className="bg-[#0F2A4A]">
              <tr>
                {['Catégorie', 'Description', 'Montant', 'Date'].map((h, i) => (
                  <th key={h} className={`${i >= 2 ? 'text-right' : 'text-left'} px-3 py-2.5 text-white font-semibold text-[10px] tracking-wide uppercase border-b border-[#0F2A4A] ${i === 0 ? 'border-l' : ''} ${i === 3 ? 'border-r' : ''}`}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
