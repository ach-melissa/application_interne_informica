...
import { X, BookOpen, DollarSign, Clock, FileText, Users, Layers, Check } from 'lucide-react';
...
const AddFormationModal = ({ onClose, onSuccess, formation = null }) => {
  ...
  const showGlobalDuree = !form.a_niveaux || (form.duree_uniforme && form.type_duree_uniforme);
  ...
  const handleSubmit = async () => {
    if (!form.nom.trim() || !form.capacite_groupe) {
      setError('Nom et capacité sont obligatoires.'); return;
    }
    if (showGlobalPrix && !form.prix) { setError('Le prix est obligatoire.'); return; }
    if (showGlobalDuree && !form.heures) { setError('La durée est obligatoire.'); return; }

    if (form.a_niveaux) {
      if (niveaux.length === 0) { setError('Ajoutez au moins un niveau.'); return; }
      for (const n of niveaux) {
        if (!n.nom.trim()) { setError('Chaque niveau doit avoir un nom.'); return; }
        if (!form.prix_uniforme && (n.prix === '' || Number(n.prix) <= 0)) { setError(`Prix requis pour le niveau "${n.nom}".`); return; }
        if (!form.duree_uniforme && (n.duree_valeur === '' || Number(n.duree_valeur) <= 0)) { setError(`Durée requise pour le niveau "${n.nom}".`); return; }
        if (!form.type_duree_uniforme && !n.type_duree) { setError(`Unité requise pour le niveau "${n.nom}".`); return; }
      }
    }
    ...
    try {
      const res = await fetch(
        ...
        { method: isEdit ? 'PATCH' : 'POST', headers: getHeaders(), body: JSON.stringify(form) }
      );
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={onClose}>
      <div className="bg-white rounded-md shadow-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-5 space-y-4">
            ...
            {form.a_niveaux && (
              <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 mt-2">
                <label className="flex items-center gap-1.5 text-[11px] text-slate-500">
                  <input type="checkbox" checked={form.prix_uniforme} className="accent-[#0369A1]"
                    onChange={e => setForm(p => ({ ...p, prix_uniforme: e.target.checked }))} />
                  Même prix
                </label>
                <label className="flex items-center gap-1.5 text-[11px] text-slate-500">
                  <input type="checkbox" checked={form.duree_uniforme} className="accent-[#0369A1]"
                    onChange={e => setForm(p => ({ ...p, duree_uniforme: e.target.checked }))} />
                  Même durée
                </label>
                <label className="flex items-center gap-1.5 text-[11px] text-slate-500">
                  <input type="checkbox" checked={form.type_duree_uniforme} className="accent-[#0369A1]"
          ...
          {/* Prix / Durée — globaux seulement s'ils sont pertinents, sinon gérés par niveau */}
          {(showGlobalPrix || showGlobalDuree) && (
            <Section>
              ...
              {showGlobalDuree && (
                <div>
                  <Label icon={Clock} text="Durée" required />
                  <div className="flex gap-1.5">
                    <select value={form.type_duree} onChange={e => setForm(p => ({ ...p, type_duree: e.target.value }))} className={`${inp} w-24 flex-shrink-0`}>
                      <option value="heures">Heures</option>
                      <option value="seances">Séances</option>
                    </select>
                    <input type="number" min="1" value={form.heures} onChange={set('heures')} className={inp}
          ...
          <div className="border-t border-[#F1F5F9] pt-3">
            <div className="flex items-center justify-between mb-2">
              <div>
                ...
                <p className="text-[10px] text-slate-400">Proposé à la création d'un groupe — modifiable ensuite.</p>
              </div>
